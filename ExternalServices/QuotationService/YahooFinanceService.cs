    ...
    public class YahooFinanceService
    {
        ...
        public async Task<decimal> GetCurrenPriceAsync()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response != null)
                {
                    YahooResponseModel result = JsonConvert.DeserializeObject<YahooResponseModel>(response);
                    var lastPriceString = result.Body.PrimaryData.LastSalePrice.Replace("$", "");
                    decimal lastPrice;
                    if (decimal.TryParse(lastPriceString, NumberStyles.Number, CultureInfo.InvariantCulture, out lastPrice))
                    {
                        return lastPrice;
                    }
                    else
